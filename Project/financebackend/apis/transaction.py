...
def get_transactions_by_time(request):
    ...
    if dropdownValue == "Year":
        ...
        transactions = Transaction.objects.all().filter(user = user1,  date_time__date__range=(lastYear, now))
    elif dropdownValue == "Month":
        lastMonth = now - timedelta(days = 30)
        transactions = Transaction.objects.all().filter(user = user1, date_time__date__range=(lastMonth, now))
    elif dropdownValue == "Day":
        lastDay = now - timedelta(days=1)
        transactions = Transaction.objects.all().filter(user = user1,  date_time__date__range = (lastDay, now))
    else:
...
def get_transaction(request, pk):
    ...
    a["is_expense"] = Category.objects.get(id = serializer.data["category"]).is_expense
    return Response(a)
