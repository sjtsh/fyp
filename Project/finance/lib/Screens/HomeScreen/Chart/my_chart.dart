...
class _BezierChartPersonalState extends State<BezierChartPersonal> {
  ...
  Widget build(BuildContext context) {
    try {
      return (context.watch<AnalysisManagement>().actualData15.values)
                  .reduce((a, b) => a + b) ==
              0
          ? Center(
              child: Text(
              "Make Transactions for analysis",
              style: TextStyle(
                  color: context.read<ThemeManagement>().allTextColor),
            ))
          : Padding(
                  ...
                  lineBarsData: [
                    LineChartBarData(
                      ...
                      spots: predictedData
                          ...
                          .entries
                          .map((content) => FlSpot(
                              content.key + 1.0, content.value + 10000.0))
                          .toList(),
                      dashArray: [5],
                      isCurved: false,
                      colors: [
                        Colors.transparent,
                        Colors.transparent,
                        context.watch<ThemeManagement>().isDark
                            ? Colors.white
                            : Colors.black,
                        context.watch<ThemeManagement>().isDark
                            ? Colors.white
      ...
      return Center(
          child: Text(
        "Make Transactions for analysis",
        style: TextStyle(color: context.read<ThemeManagement>().allTextColor),
      ));
    }
  }
}
